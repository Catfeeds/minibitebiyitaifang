...
class RenzhengController extends PublicController {
    ...
    public function saveqiye(){
        ...
        if(IS_REQUEST){
            ...
            $arr['branchbankname']=$data['branchbankname'];
            ...
            $arr['uid']=$data['uid'];
            $arr['addtime']=time();
            if(!$arr['address'] || !$arr['bankname'] || !$arr['branchbankname'] || !$arr['bankid'] || !$arr['qiyename'] || !$arr['qiyefrname'] || !$arr['qiyefr_idc'] || !$arr['qiye_cert'] || !$arr['qiyefr_idc_face'] || !$arr['qiyefr_idc_back'] || !$arr['qiye_foodcert']){
                echo json_encode(array('status'=>0,'message'=>'请将信息填完整!','respondData'=>''));
                exit();
            }
    ...
    public function savegeren(){
        ...
        if(IS_REQUEST){
            ...
            $arr['grbankname']=$data['grbankname'];
            $arr['grbranchbankname']=$data['grbranchbankname'];
            $arr['grbankid']=$data['grbankid'];
            $arr['tel']=$data['tel'];
            $arr['uid']=$data['uid'];
            $arr['addtime']=time();
            if(!$arr['grbankname'] || !$arr['grbranchbankname'] || !$arr['grbankid'] || !$arr['linkman'] || !$arr['idc_id'] || !$arr['idc_face'] || !$arr['idc_back'] || !$arr['idc_photo'] || !$arr['tel']){
                echo json_encode(array('status'=>0,'message'=>'请将信息填完整!','respondData'=>''));
                exit();
            }
            //检测记录是否已经存在，不存在则创建新的记录
        ...
        if(IS_REQUEST){
            ...
            if($info){
              if($data['dtype']=="qiye"){
                ...
                $arr['bankname']=$info['bankname'];
                $arr['branchbankname']=$info['branchbankname'];
                $arr['bankid']=$info['bankid'];
                ...
                $arr['tel']=$info['tel'];
                $arr['grbankname']=$info['grbankname'];
                $arr['grbranchbankname']=$info['grbranchbankname'];
                $arr['grbankid']=$info['grbankid'];
