...
class ApplyshopController extends PublicController{
	...
	*/
    ...
    public function join_nofinish(){
		$list=$this->join_log->where("del=0 AND status=0")->select();
		foreach ($list as $k => $v) {
			$username=M("user")->where("del=0 AND id=".$v['uid'])->getField("name");
			$list[$k]['name']=$username?$username:M("user")->where("del=0 AND id=".$v['uid'])->getField("uname");
		}
		$this->assign('list',$list);
		$this->display(); // 输出模板

	}
	public function join_finish(){
		$list=$this->join_log->where("del=0 AND status=1")->select();
		foreach ($list as $k => $v) {
			$username=M("user")->where("del=0 AND id=".$v['uid'])->getField("name");
			$list[$k]['name']=$username?$username:M("user")->where("del=0 AND id=".$v['uid'])->getField("uname");
		}
	...
	public function join_shenhe(){
		$id=I("post.id");
		if($id){
			$data['status']=1;
			$data['dealtime']=time();
			$re=$this->join_log->where("id=$id")->save($data);
			if($re){
				$this->ajaxReturn(1);
			}else{
				$this->ajaxReturn(0);
	...
	public function join_return(){
		$id=I("post.id");
		if($id){
			$re=$this->join_log->where("id=$id")->setField("del",1);
			if($re){	
				$this->ajaxReturn(1);
			}else{
				$this->ajaxReturn(0);
